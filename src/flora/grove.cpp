...
void Grove::Yield(const flora::Plant &plant, const TreeState &state, float now, float share, bool woodOnly) {
    ...
    for (const flora::DropRule &rule : def.drops) {
        ...
        if (woodOnly && !TextIsEqual(rule.item, items::kWood.name)) continue;
        ...
        // Seeded from the item's *name* rather than from its row number. A row
        // number shifts when a row is added — see `core/hash.h` — and every tree
        // already standing in the world would drop something different the day a
        // new item arrived.
        const int salt = static_cast<int>(hash::Of(rule.item) & 0x7FFFFFu);

        const float roll = Chance(plant.id, salt * 71 + 3, settings_.seed);

        if (roll >= rule.chance) continue;

        const float amount = Chance(plant.id, salt * 71 + 11, settings_.seed);
        ...
        const std::optional<Item> what = item::Named(rule.item);

        // Verify has already established that every drop names a real item, so this
        // cannot be empty in a build that started — the test is here because the
        // lookup returns an optional and silently dropping a tree's wood would be
        // worse than saying nothing.
        if (what.has_value()) drops_.Scatter(ItemsOf(*what, count), from, away, now);
    }
}
...
bool Grove::Strike(Rectangle hitbox, float damage, Vector2 from, float now) {
    for (const flora::Plant &plant : plants_) {
        ...
        if (standing.stage == flora::Stage::Sapling) {
            ...
            if (const std::optional<Item> seed = item::Named(def.sapling)) {
                drops_.Scatter(ItemsOf(*seed, 1), {plant.base.x, plant.base.y - height * 0.5f},
...
void Grove::DrawFruit(const weather::Sky &sky, flora::Season season, float now) const {
    ...
    for (const flora::Plant &plant : plants_) {
        ...
        for (const flora::DropRule &rule : def.drops) {
            if (TextIsEqual(rule.item, items::kApple.name) && rule.chance > 0.0f) bears = &rule;
        }
