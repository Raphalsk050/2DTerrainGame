...
struct DropRule {
    // The item, by name. See `mob/spoil.h` for why a name and not a handle: this
    // table is `constexpr` and an item's id is not known until startup.
        ...
        {
            {
                ...
                .drops     = {{.item = "wood", .least = 4, .most = 7, .chance = 1.0f},
                              {.item = "oak sapling", .least = 1, .most = 2, .chance = 0.55f},
                              {.item = "fibre", .least = 1, .most = 3, .chance = 0.4f}},
                .sapling   = "oak sapling",
                .deciduous = true,
            },
            {
                ...
                .drops     = {{.item = "wood", .least = 5, .most = 9, .chance = 1.0f},
                              {.item = "resin", .least = 1, .most = 2, .chance = 0.45f},
                              {.item = "pine sapling", .least = 1, .most = 2, .chance = 0.5f}},
                .sapling   = "pine sapling",
                .deciduous = false,
            },
            {
                .name        = "birch",
                ...
                .drops     = {{.item = "wood", .least = 3, .most = 5, .chance = 1.0f},
                              {.item = "birch sapling", .least = 1, .most = 2, .chance = 0.6f},
                              {.item = "fibre", .least = 1, .most = 2, .chance = 0.35f}},
                .sapling   = "birch sapling",
                .deciduous = true,
            },
            {
                .name        = "apple",
                ...
                .drops     = {{.item = "wood", .least = 2, .most = 4, .chance = 1.0f},
                              {.item = "apple", .least = 1, .most = 3, .chance = 0.7f},
                              {.item = "apple sapling", .least = 1, .most = 1, .chance = 0.5f}},
                .sapling   = "apple sapling",
                .deciduous = true,
            },
            {
                .name        = "fern",
                ...
                .drops     = {{.item = "fibre", .least = 1, .most = 2, .chance = 0.8f}},
                .deciduous = false,
            },
            {
                ...
                .drops     = {{.item = "fibre", .least = 1, .most = 3, .chance = 0.9f}},
                .deciduous = false,
            },
};
...
// The item a species sows itself from, or nothing where it does not sow.
//
// Resolved rather than stored, since the row names it and an id is only known once
// the item table has been frozen. `Verify` has already established that every name
// here resolves, so a species that names a sapling always gets one back.
inline std::optional<Item> SaplingOf(Species species) {
    return item::Named(kSpecies[SpeciesIndex(species)].sapling);
}

// The tree a sapling grows into, or nothing where the item is not a sapling.
//
// A scan of five rows, done once on a click and once a frame while a sapling is in
// hand, which is nothing at all — and it keeps the pairing stated exactly once, in
// the table, rather than as a table plus a switch somewhere that has to be
// remembered when a sixth tree arrives.
inline std::optional<Species> SpeciesOf(Item sapling) {
    for (std::size_t e = 0; e < kSpeciesCount; e++) {
        // The name test is what makes the default safe: a species with no sapling of
        // its own leaves the field at nullptr, so it can never answer for one.
        if (kSpecies[e].sapling == nullptr) continue;

        const std::optional<Item> mine = item::Named(kSpecies[e].sapling);

        if (mine.has_value() && *mine == sapling) return static_cast<Species>(e);
    }
