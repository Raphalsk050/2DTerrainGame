...
void ReportCaves(const terrain::Settings &settings, Rectangle region) {
    ...
    if (runs.empty()) {
        ...
        for (const float height : runs) {
            ...
            if (height >= player_config::Build().height) {
                upright++;
                standing += height;
            }

            if (height >= player_config::Build().crouchHeight) {
                crouch++;
                stooped += height;
            }
        }

        std::printf("clearance          (character %.0f px standing, %.0f crouched)\n", player_config::Build().height,
                    player_config::Build().crouchHeight);
...
bool probes::Headless(int argc, char **argv) {
    // The registered ones first. Anything that registers itself is answered from
    // its own row, so the two halves of the dispatch cannot disagree about it —
    // see probe_def.h. What follows is the older chain, which is still written out
    // twice and is the next thing to move.
    if (const Report *known = Named(argc, argv)) return !known->shows;
...
std::optional<int> probes::Run(int argc, char **argv, World &world, Grove &grove, terrain::Settings &settings,
    ...
    if (const Report *known = Named(argc, argv)) {
        const Bench bench = {.argc     = argc,
                             .argv     = argv,
                             .world    = &world,
                             .grove    = &grove,
                             .settings = &settings,
